...
public interface OutlayRepository extends JpaRepository<Outlay, Integer> {
    ...
    @Query(value = "select * from outlay o inner join branches b on o.branch_id = b.id where b.business_id = ?2 and o.date = ?1", nativeQuery = true)
    List<Outlay> findAllByDateAndBusinessId(java.sql.Date date,Integer business_id);
