...
//    @CheckPermission("ADD_EXCHANGE")
    ...
    //    @CheckPermission("VIEW_EXCHANGE")
    @GetMapping("/get-by-status-id/{exchangeStatus_id}")
    public HttpEntity<?> getByStatusId(@PathVariable Integer exchangeStatus_id) {
        ApiResponse apiResponse = exchangeProductBrancService.getByStatusId(exchangeStatus_id);
        return ResponseEntity.status(apiResponse.isSuccess() ? 200 : 409).body(apiResponse);
    }

    //    @CheckPermission("VIEW_EXCHANGE")
    @GetMapping("/get-by-date/{exchangeDate}")
    public HttpEntity<?> getByDate(@PathVariable Date exchangeDate) {
